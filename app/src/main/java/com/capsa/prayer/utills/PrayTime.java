...
public class PrayTime {
	...
	public PrayTime(Context context) {
		...
		timeNames.add(context.getString(R.string.fajar));
		timeNames.add(context.getString(R.string.shrooq));
		timeNames.add(context.getString(R.string.zuhur));
		timeNames.add(context.getString(R.string.asar));
		timeNames.add("Sunset");
		timeNames.add(context.getString(R.string.maghrib));
		timeNames.add(context.getString(R.string.isha));
	...
	public ArrayList<PrayerTime> getPrayerTimes(UserLocation location, AppManager appManager, Calendar date) {
		int juristic = appManager.getJuristic();
		...
		this.setTimeFormat(Time12);
		this.setCalcMethod(appManager.getCalcMethod());

		if (juristic != 1) {
			this.setAsrJuristic(0);
		} else {
			this.setAsrJuristic(appManager.getJuristic());
		}

		this.setAdjustHighLats(AngleBased);

		int[] offsets = { 0, 0, 0, 0, 0, 0, 0 }; // {Fajr,Sunrise,Dhuhr,Asr,Sunset,Maghrib,Isha}
		this.tune(offsets);

		ArrayList<String> prayerTimes = this.getPrayerTimes(date, latitude, longitude, this.timeZone);
		ArrayList<String> prayerNames = this.getTimeNames();

		for (int index = 0; index < prayerTimes.size(); index++) {
			PrayerTime object = new PrayerTime();
			object.setPrayerName(prayerNames.get(index));
			object.setPrayerTime(prayerTimes.get(index));
			object.setAlarm(appManager.isAlarmSet(object.getPrayerName()));
			if (index != 4) {
				if (object.isAlarmSet()) {
					appManager.setAlarm(object.getPrayerTime(), (index + 1));
				}
				list.add(object);
			}
	...
	/*
	  ...
	  @param juristic
	  @return list : Prayer Time list containing information for prayer times
	          and names
	 */
	public ArrayList<PrayerTime> getPrayerTimes(UserLocation location, AppManager appManager) {
		int juristic = appManager.getJuristic();
		...
		ArrayList<String> prayerTimes = this.getPrayerTimes(cal, latitude, longitude, this.timeZone);
		ArrayList<String> prayerNames = this.getTimeNames();
