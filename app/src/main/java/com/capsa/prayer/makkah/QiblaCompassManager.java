...
package com.capsa.prayer.makkah;
...
import com.capsa.prayer.activtites.QiblaActivity;
...
public class QiblaCompassManager implements SensorEventListener, LocationListener, ConstantUtilInterface {
	...
	private final Handler mHandler = new Handler() {
		...
		public void handleMessage(Message message) {
			...
			case MESSAGE_ON_DEVICE_HEAD_DIRECTION:
				...
				try {
					Double deviceDirection = (Double) message.getData().get(MESSAGE_NORTH_DIRECTION_ANGEL_KEY);

					changeNorthDirection(deviceDirection.doubleValue());
					lastNorthAngle = deviceDirection.doubleValue();
					isNorthChanged = true;
					qiblaActivity.signalForAngleChange(deviceDirection, false);
			...
			case MESSAGE_ON_QIBLA_DIRECTION:
				Double qiblaDirection = (Double) message.getData().get(MESSAGE_DEVICE_LOCATION_KEY);
				changeQiblaDirection(qiblaDirection.doubleValue());
				lastQiblaAngle = qiblaDirection.doubleValue();
				isQiblaChanged = true;
				qiblaActivity.signalForAngleChange(qiblaDirection, true);

				break;
			default:
				Log.d(NAMAZ_QIBLA_LOG_TAG, "Unhandled Message for QiblaCompassManager message.what=" + what);
			}
		}
	};
	...
	private boolean isFarEnough(Location newLocation, Location prevLocation) {
		...
		double a = Math.sin(deltaLat / 2) * Math.sin(deltaLat / 2) + Math.cos(Math.toRadians(prevLatitude)) * Math.cos(Math.toRadians(newLatitude)) * Math.sin(deltaLong / 2) * Math.sin(deltaLong / 2);
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (mCount++ > 50) {
			...
			if (previousNorth == null || isDeltaDegreeEnough(new Double(newDegree), previousNorth)) {
				Message message = mHandler.obtainMessage();
				message.what = MESSAGE_ON_DEVICE_HEAD_DIRECTION;
				Bundle bundle = new Bundle();
				bundle.putDouble(MESSAGE_NORTH_DIRECTION_ANGEL_KEY, -(newDegree));
	...
	private boolean isDeltaDegreeEnough(Double newDegree, Double previousDegree) {
		...
		if (delta > MIN_DEGREE_FROM_NORTH && delta < (360 - MIN_DEGREE_FROM_NORTH)) {
			return true;
		} else {
			return false;
		}
...
class LocationChangedAsyncTask extends AsyncTask<Location, Void, Double> {
	...
	protected Double doInBackground(Location... params) {
		...
		double northDirection = QiblaDirectionCalculator.getQiblaDirectionFromNorth(params[0].getLatitude(), params[0].getLongitude());
		
		return Double.valueOf(northDirection);
	}
	...
	protected void onPostExecute(Double result) {
		...
		Bundle bundle = new Bundle();
		bundle.putDouble(QiblaCompassManager.MESSAGE_DEVICE_LOCATION_KEY, result.doubleValue());
		message.setData(bundle);
