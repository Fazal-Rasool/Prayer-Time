...
public class QiblaDirectionCalculator {
	public static final double QIBLA_LATITUDE = Math.toRadians(21.422659);
	public static final double QIBLA_LONGITUDE = Math.toRadians(39.826147);
	...
	public static double getDistance(double latitude, double longitude) {
		...
		double $a = Math.pow(Math.sin($dlat / 2), 2) + Math.cos(latitude) * Math.cos(QIBLA_LATITUDE) * Math.pow((Math.sin($dlon / 2)), 2);
	...
	public static double getBearing(double degLatitude, double degLongitude) {
		double lonDelta = (QIBLA_LONGITUDE - degLongitude);
		double y = Math.sin(lonDelta) * Math.cos(degLatitude);
		double x = Math.cos(QIBLA_LATITUDE) * Math.sin(degLatitude) - Math.sin(QIBLA_LATITUDE) * Math.cos(degLatitude) * Math.cos(lonDelta);
		double bearing = Math.toDegrees(Math.atan2(y, x));
		AppUtils.printLog("", "bearing -> " + bearing);
		return bearing;
	}
